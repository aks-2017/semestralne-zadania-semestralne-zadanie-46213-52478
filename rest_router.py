...
class RestRouterAPI(app_manager.RyuApp):
    ...
    def datapath_handler(self, ev):
        if ev.enter:
            ...
            def posli_adresy(zariadenia):
                ...
                self.adresy = zariadenia

            def posli_routy(zariadenia):
                for id_zariadenia, val in zariadenia.items():
                    device_id = id_zariadenia
                    for siet, gw in val.items():
                        print set_routes(siet, gw)
                        print "[Cont] ", call_curl(set_routes(siet, gw), device_id)

            # RouterController.set_data_command(mapper.connec,"0000000000000002", dict(address="10.1.2.1/24"))
            # print "-X POST -d '{\"address\": \"10.1.2.1/24\"}' http://localhost:8080/router/0000000000000002"
            zariadenia2 = {"0000000000000001": ["172.16.0.1/30", "10.1.1.1/24"],
                           "0000000000000002": ["172.16.0.2/30", "10.1.2.1/24", "10.1.3.1/24"]}

            zariadenia4 = {"0000000000000001": ["172.16.0.1/30", "10.1.1.1/24", "172.16.0.5/30"],
            ...
            # print "oneeeeeeeeeeeeeeeeeeeeee"
        else:
    ...
    def link_change_handler(self, ev):
        ...
        def get_pair_of_ip(pole1, pole2):
            for adresa1 in pole1:
                ipcka1 = IPNetwork(adresa1)
                netvor = ipaddress.ip_network(unicode(str(ipcka1.network) + "/" + str(ipcka1.prefixlen)))
                for adresa2 in pole2:
                    # print adresa2[:-3], "   ", netvor, ipaddress.ip_address(unicode(adresa2[:-3])) in netvor
                    if (ipaddress.ip_address(unicode(adresa2[:-3])) in netvor):
                        # print "totok: "
        ...
        def del_routes(route_id):
            return "\"route_id\": \"" + route_id + "\""

        def call_curl(action, obsah, device_id):
        ...
        def insert_to_routes_list(routes_list, device_id, prvok):
            if device_id in routes_list:
                temp = routes_list.get(device_id)
                temp[prvok.keys()[0]] = prvok.values()[0]
                # print prvok.keys()[0], prvok.values()[0]
                routes_list[device_id] = temp
                # print "temp:   ", temp
            else:
        ...
        if not self.handluj:
            ...
            # Tu sa riesi zistovanie liniek
            self.handluj = False
            print "oneeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
            ...
            # #self.net.add_edges_from([(99, 1, {'port': 1}), (1, 99, {'port': 2})])
            ...
            #     print "porovnavam", li.src.dpid, " a ", li.dst.dpid
            #     if li.src.dpid < li.dst.dpid:
            #         filtered_links_list.append(li)
            links_f = [(link.src.dpid, link.dst.dpid) for link in links_list]
            print "vytriedene", links_f
            # self.net.remove_edges_from(links)
            self.net.add_edges_from(links_f)
            # Nasledujuce dva riadky su pridanie hostov do topologie - kedze ich nechce discovernut
            self.net.add_edges_from([(99, 1, {'port': 1}), (1, 99, {'port': 2})])
            self.net.add_edges_from([(100, 2, {'port': 1}), (2, 100, {'port': 2})])
            print self.net.edges()
            print "**********List of links"
            for link in filtered_links_list:
                ...
                self.no_of_links += 1
            print "konieeeeeeeec"
            path = nx.shortest_path(self.net, 99, 100)
            print "path: ", path

            routes_list = {}
            # Prechadzame iteratorom cely slovnik
            for current_item, next_item in izip(path, islice(path, 1, None)):
                print "jjj", (current_item, next_item)
                # najdeme zaznam v poli s info o adresach
                for key, item in self.adresy.items():
                    if current_item == int(key):
                        # print key
                        # najdeme nasledujuci zaznam v poli s info o adresach
                        for keyv, itemv in self.adresy.items():
                            if next_item == int(keyv):
                                # print keyv
                                print "jano", key, " ", item, " a ", keyv, " ", itemv
                                pair = get_pair_of_ip(item, itemv)
                                if pair is not None:
                                    print pair
                                    data = {"10.1.2.0/24": pair[1][:-3]}
                                    print "idem inzertuvat"
                                    insert_to_routes_list(routes_list, key, data)
                                    data = {"10.1.1.0/24": pair[0][:-3]}
                                    insert_to_routes_list(routes_list, keyv, data)
...
class RouterController(ControllerBase):
    ...
    # @classmethod
    # def set_data_command(self, req, switch_id):
    #     print "*********************************111111111"
    #     print "req: ", req
    #     print "switch_id: ", switch_id
    #     print "*********************************111111111"
    #     return self._access_router(switch_id, VLANID_NONE, 'set_data', req)
